...
export default class DataObj extends CanvasShape {
  public name = 'DataObj';
  ...
  private createPath2D() {
    ...
    path.moveTo(this.x + 0.2*this.width + 1, this.y - this.height);
    ...
    path.lineTo(this.x + 0.2*this.width, this.y - this.height + 0.8*this.width);
    path.lineTo(this.x + this.width, this.y - this.height + 0.8*this.width);
